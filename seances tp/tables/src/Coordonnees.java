	...
	*/
	public static boolean segmentsCroises(Coordonnees p1, Coordonnees p2,
		...
		if(p1Long==p2Long && q1Long==q2Long){
			...
			// Au moins partiellement superposes
			if(Math.min(q1Lat,q2Lat)>=Math.min(p1Lat,p2Lat) && Math.min(q1Lat,q2Lat)<=Math.max(p1Lat,p2Lat))
				return true;
			if(Math.min(p1Lat,p2Lat)>=Math.min(q1Lat,q2Lat) && Math.min(p1Lat,p2Lat)<=Math.max(q1Lat,q2Lat))
				return true;
			return false;
		}
