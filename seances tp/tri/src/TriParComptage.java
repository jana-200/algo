...
public class TriParComptage {
    ...
    //   on peut aussi utiliser la méthode triSelf (int[] t, int borneInf, int borneSup) --> triSelf(t,0,borneSup);
	}
   ...
   public static void triSelf (int[] t, int borneInf, int borneSup) {
	   int[] count = new int[borneSup+Math.abs(borneInf)+1];
	   int indiceDansT=0;
	   for (int i = 0 ; i < t.length; i++) {
		   count[t[i]+Math.abs(borneInf)]++;
	   }
	   for (int i = 0; i < count.length; i++) {
		   for (int j = 0; j < count[i]; j++) {
			   t[indiceDansT]=i-Math.abs(borneInf);
			   indiceDansT++;
		   }
	   }
   }
